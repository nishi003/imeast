...
const Index = ({ lesson, title, module, setVideo, video }) => {
    return (
        <>
            {video === -1 ?
                <Link className='flex flex-row w-full items-start gap-6' onClick={() => { setVideo(lesson) }} to={`/user/module/${module}/video/${lesson}/`}>
                ...
                :
                <Link className='flex flex-col gap-1' onClick={() => { setVideo(lesson) }} to={`/user/module/${module}/video/${lesson}/`}>
